...
searchByName(search: string) {
  const noFilterTable:IUserInterface[] = [...this.users()];
  const filterTable: IUserInterface[] = [...noFilterTable].filter(d => 
    d.name.toLowerCase().indexOf(search.toLowerCase()) > -1
  );
  if (search) {
    return this.users.set(filterTable);
  }
  return this.users.set(noFilterTable);
}

  getName() {
    const noFilterTable:IUserInterface[] = [...this.users()];
    this.signalTableForm.valueChanges.pipe(delay(1000)).subscribe(d => {
      if (d.search_name === '') {
      return  this.users.set(noFilterTable);
      }
